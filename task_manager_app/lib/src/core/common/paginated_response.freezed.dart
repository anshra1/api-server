...
extension PaginationInfoPatterns on PaginationInfo {
  ...
  TResult maybeWhen<TResult extends Object?>(
    TResult Function(int page, int limit, int total, int totalPages)?
        $default, {
    required TResult orElse(),
  }) {
    final _that = this;
    switch (_that) {
      case _PaginationInfo() when $default != null:
        return $default(_that.page, _that.limit, _that.total, _that.totalPages);
      case _:
        return orElse();
    }
  }
  ...
  TResult when<TResult extends Object?>(
    TResult Function(int page, int limit, int total, int totalPages) $default,
  ) {
    final _that = this;
    switch (_that) {
      case _PaginationInfo():
...
class _PaginationInfo implements PaginationInfo {
  const _PaginationInfo(
      {this.page = 1, this.limit = 20, this.total = 0, this.totalPages = 0});
...
abstract mixin class $PaginatedResponseCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({List<T> items, PaginationInfo pagination});
...
extension PaginatedResponsePatterns<T> on PaginatedResponse<T> {
  ...
  @optionalTypeArgs
  TResult maybeWhen<TResult extends Object?>(
    TResult Function(List<T> items, PaginationInfo pagination)? $default, {
    required TResult orElse(),
  }) {
    final _that = this;
    switch (_that) {
      case _PaginatedResponse() when $default != null:
        return $default(_that.items, _that.pagination);
      case _:
        return orElse();
    }
  }
